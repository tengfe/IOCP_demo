...
CIOCPServer::CIOCPServer()
{
	...
	::InitializeCriticalSection(&m_CloseOrErrLock);	// [2015.9.1 add]
...
CIOCPServer::~CIOCPServer()
{
	...
	::DeleteCriticalSection(&m_CloseOrErrLock); // [2015.9.1 add]
...
void CIOCPServer::ReleaseBuffer(CIOCPBuffer *pBuffer)
{
	...
	if (m_nFreeBufferCount < m_nMaxFreeBuffers)	// 将要释放的内存添加到空闲列表中 [2015.11.15 mod]old:m_nFreeBufferCount <= m_nMaxFreeBuffers
	{
...
CIOCPContext *CIOCPServer::AllocateContext(SOCKET s)
{
	...
	else
	{
		...
		m_nFreeContextCount--; // [2015.8.9 mod][old: m_nFreeBufferCount--] 
	}
...
void CIOCPServer::ReleaseContext(CIOCPContext *pContext)
{
	...
	if (m_nFreeContextCount < m_nMaxFreeContexts) // 添加到空闲列表 [2015.11.10 mod][old: m_nFreeContextCount <= m_nMaxFreeContexts]如果m_nFreeContextCount==m_nMaxFreeContexts时，会在下一次导致m_nFreeContextCount>m_nMaxFreeContexts
...
void CIOCPServer::CloseAConnection(CIOCPContext *pContext)
{
	...
	if (pTest == pContext)
	{
		m_pConnectionList = pTest->pNext; // [2015.11.9 mod Lostyears][old: m_pConnectionList =  pContext->pNext]
		m_nCurrentConnection--;
	}
	else
	{
...
BOOL CIOCPServer::RemovePendingAccept(CIOCPBuffer *pBuffer)
{
	...
	{
		m_pPendingAccepts = pTest->pNext; // [2015.8.9 mod][old: m_pPendingAccepts = pBuffer->pNext]
		bResult = TRUE;
	}
...
CIOCPBuffer *CIOCPServer::GetNextReadBuffer(CIOCPContext *pContext, CIOCPBuffer *pBuffer)
{
	if (pBuffer != NULL)
	{
		...
		else			// 应该插入到表的中间
		{
			...
			pPre->pNext = pBuffer; // [2015.8.9 mod][old: pPre->pNext = pBuffer->pNext]
		}
	}
...
DWORD WINAPI CIOCPServer::_WorkerThreadProc(LPVOID lpParam)
{
	...
	while (TRUE)
	{
		...
		pBuffer = CONTAINING_RECORD(lpol, CIOCPBuffer, ol); // [2015.8.9 bak][lpol作为CIOCPBuffer的ol成员，由其地址取CIOCPBuffer实例首地址]
		int nError = NO_ERROR;
		if (!bOK)						// 在此套节字上有错误发生
		{
			SOCKET s;
...
void CIOCPServer::HandleIO(DWORD dwKey, CIOCPBuffer *pBuffer, DWORD dwTrans, int nError)
{
	...
	if (pContext != NULL)
	{
		...
		{
#ifdef _DEBUG
			::OutputDebugString(L"	检查到套节字已经被我们关闭 \n");
#endif // _DEBUG
				...
				if (AddAConnection(pClient))
				{
					...
					else
					{
						...
						else
						{
							...
							for (int i = 0; i<m_nInitialReads; i++) // [2015.8.21 mod][将常量值改为m_nInitialReads]
							{
					...
					//
					//// 向新连接投递几个Read请求，这些空间在套节字关闭或出错时释放
					//for(int i=0; i<m_nInitialReads; i++) // [2015.8.22 mod][old: i<5]
					//{
					//	CIOCPBuffer *p = AllocateBuffer(BUFFER_SIZE);
					//	if(p != NULL)
					//	{
